#!/usr/bin/env python3
"""
Strava client variant that uses the shared generic API client (api_utils.py)
for the OAuth token request, then fetches activities and exports them.
"""
...
from python_utils.classes.integrations.api_client.api_utils import call_api
...
def get_access_token() -> str:
    """
    Exchange the Strava refresh token for an access token using api_utils.call_api.

    Requires env vars:
        API_BASE_URL          (e.g. https://www.strava.com/api/v3)
        STRAVA_CLIENT_ID
    ...
    # Strava token endpoint expects form-encoded data
    data = call_api("POST", "/oauth/token", payload=payload, as_form=True)
...
def fetch_activities(
    ...
    """
    Fetch activities using requests, with pagination and simple 429 backoff.

    Uses API_BASE_URL from environment for consistency with api_utils.py.
    """
    api_base = os.getenv("API_BASE_URL")
    if not api_base:
        logger.error("API_BASE_URL must be set (e.g. https://www.strava.com/api/v3) in the environment or .env file.")
        sys.exit(1)
