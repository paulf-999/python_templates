...
class VariableUtils:
    ...
    def load_ip_params_from_config(self, config_file_path):
        ...
        self.logger.debug("Function called 'VariableUtils.load_ip_params_from_config()'")
    ...
    def setup_directory_vars(self, PROJECT_DIR):
        ...
        self.logger.debug("Function called 'VariableUtils.setup_directory_vars()'")
        ...
        if root_index != -1:
            ...
            for var_name, var_value in zip(list_dir_vars_str, list_dir_vars):
                self.logger.debug(f"{var_name} = {var_value}")
            # fmt: on

            return SCRIPT_DIR, SRC_DIR, IP_DIR
        else:
            self.logger.debug("Error: Project root not found in script path.")
